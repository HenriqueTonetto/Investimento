...
document.getElementById("calcularButton").addEventListener("click", function () {
  ...
  const tipoTempo = document.querySelector("input[name='tipoTempo']:checked").value;

  const meses = tipoTempo === 'ano' ? periodo * 12 : periodo;
  const taxaMensal = tipoTempo === 'ano' ? Math.pow(1 + (taxaJuros / 100), 1/12) - 1 : taxaJuros / 100;
